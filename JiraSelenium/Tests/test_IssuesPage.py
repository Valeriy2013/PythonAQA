...
class TestIssuesPage(TestTemplate):
    ...
    def test_search_issue(self):
        ...
        self.issues_page.create_issue(criteria)
        assert self.issues_page.is_issue_created()[0]
    ...
    @pytest.mark.parametrize('updated_issue', [
        ...
        Issue('Webinar (WEBINAR)', 'Bug', 'Bug for update test', 'Highest', 'webinar1')
    ])
    @pytest.mark.flaky(reruns=3)
    def test_update_issue(self, updated_issue):
        issue = Issue('Webinar (WEBINAR)', 'Bug', 'Bug for update test', 'Highest', 'ValeriiSokolovskyi')
        self.issues_page.create_issue(issue)
        assert self.issues_page.is_issue_created()[0]
        issue_link = self.issues_page.is_issue_created()[1]
        self.issues_page.update(issue, updated_issue)
